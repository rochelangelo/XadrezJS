...
function possiveis_movimentos() {
    ...
    document.getElementById('t' + x + y).style.backgroundColor = "#00FFFF"; //muda cor de fundo
    possiveis[c] = "t" + x + y;
    c++;
    ...
    function possivel(px, py) {
        if (px > 0 && px < 9 && py > 0 && py < 9 && peca[px][py]['cor'] != movimenta['selecionada']['cor']) {
            document.getElementById('t' + (px) + (py)).style.backgroundColor = "#00FFFF"; //muda cor de fundo
            possiveis[c] = "t" + (px) + (py);
            c++;
...
function seleciona(x, y) {
    ...
    if ((movimenta['selecionada']['x'] == 0 || peca[x][y]['cor'] == movimenta['selecionada']['cor']) && peca[x][y]['cor'] == vez) {
        ...
        if (peca[x][y]['peca'] == "rei") {
            ...
            window.location.replace("http://localhost:8383/XadrezLpw/novoJogo.html");
